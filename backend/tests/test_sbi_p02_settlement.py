...
def test_settlement_matched_when_p01_matched_by_deduct_date(db):
    ...
    bid = _settlement(db, "1A999003", "2026-07-25", "2026-07-27", 23600)
    db.add(SBIP01Result(recon_date="2026-07-25", ko_id="1A999003", status="matched",
...
def test_settlement_pending_without_p01_or_kowdl(db):
    bid = _settlement(db, "1A999003", "2026-07-25", "2026-07-27", 23600)   # no P01, no same-day KO wdl
...
def test_settlement_matches_same_day_kowdl_without_p01(db):
    ...
    bid = _settlement(db, "1A999003", "2026-07-27", "2026-07-27", 23600)
    db.add(SBIKOLimits(txn_date="2026-07-27", ko_id="1A999003", txn_type="KO Withdrawal", amount=23600))
    db.commit()
    assert _status(db, "2026-07-27", bid) == "Matched (Settlement)"
...
def test_p01_unmatched_does_not_falsely_match(db):
    bid = _settlement(db, "1A999003", "2026-07-25", "2026-07-27", 23600)
    db.add(SBIP01Result(recon_date="2026-07-25", ko_id="1A999003", status="unmatched",
