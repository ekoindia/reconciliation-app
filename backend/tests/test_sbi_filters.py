...
def test_p01_ko_search(db):
    db.add(SBIP01Result(recon_date=RD, ko_id="1A850086", status="CREDITED"))
    db.add(SBIP01Result(recon_date=RD, ko_id="1A850143", status="PARTIAL"))
    db.commit()
    out = get_p01_results(recon_date=RD, ko_id="0086", db=db, current_user=USER)
    assert [r["ko_id"] for r in out["rows"]] == ["1A850086"]
