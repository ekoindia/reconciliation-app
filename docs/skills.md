...
| A backend 422's array `detail` fed to a toast/JSX crashes the WHOLE app to blank | A form sent `src_note: <v>?.trim() \|\| null` (null on an empty field) to a schema typed `src_note: str` — Pydantic 422 whose `detail` is an ARRAY of `{type,loc,msg,input,url}` objects. Every catch did `toast.error(e.response.data.detail \|\| '…')`, handing react-hot-toast an object → React error #31 "Objects are not valid as a React child". Because `<Toaster>` sits OUTSIDE the route ErrorBoundary, the crash unmounts the entire SPA → pure blank (not the boundary's card). Reproduced only with an empty optional field, so it looked like "every entry" | Two fixes, both needed. (1) Root: an optional text field must be `Optional[str] = ""` (or the client must send `''`, never `null`) so a blank input doesn't 422 — mirror it across every product's copy of the schema, not just the one reported. (2) Systemic, one place: in the axios response-error interceptor, flatten a non-string `detail` (Pydantic array/object) to a string BEFORE it reaches any caller — `Array.isArray(d) ? d.map(x=>x.msg).join('; ') : (d.msg\|\|JSON.stringify(d))`. That immunises all ~120 `toast.error(...detail...)` sites at once (grep to size the blast radius; confirm none consume `detail` structurally first). Consider also placing `<Toaster>` INSIDE the ErrorBoundary so a stray non-string toast degrades to the card, not a blank app. Same family as the enum-crash row — an unhandled shape rendered as a React child |
...
| A stable overlay key with NO natural business identifier | SBI SRC tags key a P02 bank row on `reference_number`, but cash-deposit rows ("CSH DEP (CDM)…") carry no reference — so `_src_key` returned None and Assign SRC 400'd "no stable key to tag" (which, before the toast/interceptor fix, blanked the whole app). A tempting fallback — a composite of the columns that ARE present (amount, date, type) — FANS OUT: two same-amount cash deposits share it, so tagging one flips both | When a row has no business id, fall back to the row's surrogate id (here `bank_txn_id`) namespaced (`btxn|…`) so it's UNIQUE per row (no fan-out) — accept the one weakness (a surrogate id regenerates on a file RE-UPLOAD, unlike stable content, so the tag is lost then and the operator re-tags; there's no stabler key for a ref-less row). Thread that id through EVERY path that computes the key — the assign endpoint, the live read overlay (via the result handle), the results tab, AND the export recs — or the tag shows in one surface and not another. But keep it OUT of the export's OUTPUT columns: two export builders exist here (one writes `_SBI_EXPORTS` cols straight to the sheet, the daily-pack writes `_P0x_XCOLS`), so enrich the *rec* like `bank_description` is (available to the key function) rather than adding it to the loaded-columns list — that leaks the raw id into the operator's workbook and broke the empty-export header test. Same family as the fan-out row above |
...
| Tagging a per-ROW disposition through an AGGREGATE result — one tag lands on many rows | SRC tags on SBI keyed off the P0x *result* row, but a P01 result is one KO-**DAY** aggregate whose key was `ko_id` alone. Every KO Withdrawal and Bank Settlement of that KO on that date therefore shared one key AND one result_id, so tagging one row silently tagged its siblings ("1 transaction pe SRC tag kar rha hu to dusre me automatic ho ja rha hai"). Measured before fixing: 214 of 231 P01-backed rows sat on a shared key — the operator saw only the two in front of them | A disposition is per ROW; it may only be keyed by something that identifies ONE row. Where the recon result is an aggregate (per-KO, per-day, per-CSP), do NOT reuse it as the tag key — give rows that own a physical source line their own namespace + CONTENT key (`ko|amount|date|datetime`; for bank lines add the running balance as the discriminator). Keep that SRC identity in SEPARATE fields (`src_process`/`src_id`) from the matching identity (`result_process`) — other logic keys on the latter (the pair-picker's exclusion set), so overloading it silently changes matching. Compute the key once at build time and reuse it at read time so write and read can't diverge. Leave the aggregate-level screen (the P01 tab) tagging at its own granularity — different screens legitimately tag different things. Prove it on REAL data by tagging one row of a known sibling pair and asserting the sibling stays clean (then remove the test tag) |
| Counting a row as OPEN without giving it any ACTION | After making unpaired KO Deposits count as open (so two screens agreed), those rows rendered with an empty Action column — no select checkbox, no "match", no "src" — because every control keys off `result_process`, which the builder only stamps when the row maps to a P0x RESULT row. A deposit has no result row (it is the counterpart you pair against), so it became a visible dead end: "no option of selection, neither the SRC nor match option". An audit then showed the same dead-end hit the LARGEST open bucket too — P01-backed rows (KO Withdrawals, 376 of 398 open rows) never got a match link because the link was gated on `['p02','p03']` | Anything you count as open must be actionable in the same change — "shows in the count" and "can be worked" are one feature, not two. Two moves: (1) for a source row with no result, tag it with its OWN namespace (`kod`) and key it on stable CONTENT (ko+amount+date+datetime) rather than a surrogate id — content survives re-runs AND re-uploads, and the datetime discriminator stops a tag fanning out across two same-amount rows; funnel every write path (assign / assign-bulk / remove / remove-bulk) through ONE resolver so they can't drift, and watch for the source table lacking the column the result tables have (SBIKOLimits has `txn_date`, not `recon_date` — a naive `row.recon_date` AttributeErrors). (2) where a row reconciles as a PAIR rather than a single result (P01, deposits), don't force a one-sided match modal — deep-link to the pair-picker pre-filtered to that row's date+amount. Audit by listing every open row whose action-gating field is unset, per source, on REAL data — that is what surfaced the 376-row P01 gap that no one had reported |
| Two screens over the SAME date showing different "open" counts — each internally correct | The pair-picker showed 33 open for a date; the All-Entries view showed 24. Both were "right": the picker included KO Deposits (the counterpart you pair against, excluded from the exceptions view) and deferred the bank side to the reconciliation report, while All-Entries excluded deposits and let a WEAK (csp/ko, amount) P03 overlay mark two report-open credits as matched. Operators don't see scope — they see one date with two numbers and stop trusting the tool ("it's confusing") | Same entity + same date must yield ONE number, whatever the screen. Don't "explain" the gap in a tooltip — pick the HONEST definition and make every surface use it: the union that hides nothing (here: report-open bank + open withdrawals + unpaired deposits), never the smaller number that reconciles by hiding real open items. Give the previously-uncounted rows a real status (an unpaired deposit is genuinely open — 594/600 manual pairs consume one) rather than leaving them status-less and filtered out. Expect the headline to RISE and get sign-off first, since it restates a KPI. Then prove equality on real data across several dates (both sides, per date), and check the third reader (the dashboard) lands on the same figure — here it already did, so no change; had it not, it would need the same definition. Two tests that asserted the OLD divergence are the tell that you're changing behaviour deliberately: rewrite them to assert agreement |
| A read-time overlay reaches the live view AND the report engine, but the **exec/analytics dashboard is a THIRD reader** you forgot | Manual pairs were applied in the unified view and then (a later fix) in the downloadable report's `reconcile()` — but the analytics dashboard aggregates the P0x **result tables directly** with GROUP-BY and applies no overlay, so operator manual matching never moved the dashboard's SBI unmatched count ("auto-match left 30, I manually matched them, still 30"). Reader-drift bugs recur one reader at a time: fixing view→report still left view+report≠dashboard | Enumerate EVERY reader that buckets the same rows — live view, report engine, AND the analytics/exec aggregator — and land them together. For an aggregate reader that never materialises rows (it GROUP-BYs for speed over a 100k-row table), don't rebuild the full overlay per date (measured 3.8s×25 dates ≈ 95s → HTTP timeout); instead compute a cheap **delta** by recomputing only the few-thousand OPEN rows' overlay keys with the SAME `_pair_key`/`_row_disc` the overlay was written with (zero divergence), and reclassify the aggregate: subtract from unmatched, add to matched, move the ₹ too. Keep it a strict no-op when no overlay rows exist (dashboard byte-identical until the first pair) and prove the cheap delta equals the full-rebuild ground truth on real data before shipping (here 600 total, one date 33→12 = exactly the operator's expected number). `min(delta, unmatched)`-guard so it can never over-reduce. Co-locate the delta helper WITH the overlay's key functions (one source of truth), analytics just consumes the number |
